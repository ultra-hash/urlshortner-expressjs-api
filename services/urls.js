...
module.exports = (config) => {
  ...
  const { getRowsByShortUrl, getUrlsByUsernameAndCount } =
    QueryAnalytics(config);
  ...
  async function createShortUrl(req, res) {
    ...
    if (
      ...
      !["http:", "https:"].includes(newUrl.protocol.toLowerCase())
    )
      return res
        .status(400)
        .json({ error: "protocal has to be http:// or https://" });
  ...
  async function getShortUrlStats(req, res) {
    ...
    try {
      ...
      for (let eachRow of entries) {
        userAgentsAndVisits[eachRow.user_agent]
          ? userAgentsAndVisits[eachRow.user_agent]++
          : (userAgentsAndVisits[eachRow.user_agent] = 1);
        ipAddressAndVisits[eachRow.ipaddress]
          ? ipAddressAndVisits[eachRow.ipaddress]++
          : (ipAddressAndVisits[eachRow.ipaddress] = 1);

        const time = new Date(eachRow.created_at).getHours();
        const nextHour = time === 23 ? 0 : time + 1;
        activityTimeAndVisits[`${time}:00 - ${nextHour}:00`]
          ? activityTimeAndVisits[`${time}:00 - ${nextHour}:00`]++
          : (activityTimeAndVisits[`${time}:00 - ${nextHour}:00`] = 1);
      }
