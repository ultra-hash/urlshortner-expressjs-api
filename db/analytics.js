module.exports = (config) => {
  ...
  async function getRowByIpAddress(ipaddress) {
    const [row] = await mysqlCient.query(
      `SELECT * FROM ipaddresses WHERE ipaddress = ?`,
      [ipaddress]
    );
    return row[0];
  }
  ...
  async function setRowByIpaddress(ipaddress) {
    const [result] = await mysqlCient.query(
      `INSERT INTO ipaddresses (ipaddress) VALUES (?)`,
      [ipaddress]
    );
    return result;
  }
  ...
  async function getRowByUserAgent(userAgent) {
    const [row] = await mysqlCient.query(
      `SELECT * FROM user_agents  WHERE user_agent = ?`,
      [userAgent]
    );
    return row[0];
  }
  ...
  async function addAnalytics(urlId, ipaddressId, userAgentId) {
    const [result] = await mysqlCient.query(
      `INSERT INTO analytics (url_id, ipaddress_id, user_agent_id) VALUES (?, ?, ?)`,
      [urlId, ipaddressId, userAgentId]
    );
    return result;
  }

  async function getRowsByShortUrl(shortUrl) {
    const [rows] = await mysqlCient.query(
      `select t2.short_url, t2.long_url, t3.user_agent , t4.ipaddress, t1.created_at
        ...
        LEFT JOIN ipaddresses as t4 ON t1.ipaddress_id = t4.id`,
      [shortUrl]
    );
    return rows;
  }
  ...
  async function addChangeInUserDetails(valuesArray) {
    ...
    valuesArray.forEach(async (value) => {
      const [rows] = await mysqlCient.query(
        `INSERT INTO user_details_history (user_id, user_column_name, user_column_type, previous_value ) VALUES (?, ?, ?, ?)`,
        [...value]
      );
      result.push(rows);
    });
  ...
  async function getUrlsByUsernameAndCount(username) {
    const [rows] = await mysqlCient.query(
      `SELECT short_url, long_url, count(t2.url_id) as count, t1.created_at as created_at
  ...
  async function countNewUsersLast24Hours() {
    const [row] = await mysqlCient.query(
      `SELECT count(*) AS total_users FROM users WHERE created_at between SUBDATE(now(), 1) AND now()`
    );
    return row;
  }
  ...
  async function countNewUrlsLast24Hours() {
    const [row] = await mysqlCient.query(
      `SELECT count(*) AS total_urls FROM urls WHERE created_at between SUBDATE(now(), 1) AND now()`
    );
    return row;
  }
  ...
  async function countTotalHits() {
    const [row] = await mysqlCient.query(
      `SELECT count(*) AS total_hits FROM analytics`
    );
    return row;
  }
  ...
  async function countNewHitsLast24Hours() {
    const [row] = await mysqlCient.query(
      `SELECT count(*) AS total_hits FROM analytics WHERE created_at between SUBDATE(now(), 1) AND now()`
    );
    return row;
  }
